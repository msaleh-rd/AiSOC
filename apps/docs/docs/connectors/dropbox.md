...
sidebar_position: 79
title: Dropbox
description: Dropbox Business team audit log events into Intelligence SOC via /2/team_log/get_events.
---

# Dropbox

The Dropbox connector polls the **/2/team_log/get_events** endpoint of
the Dropbox Business API and emits one Intelligence SOC alert per team event
...
4. **Settings tab → OAuth 2** : add a redirect URI (only required if you
   ...
   **Generate** under *Generated access token* to get a long-lived
   team-admin token.
5. Copy the token.
6. In Intelligence SOC: **Connectors → Add connector → Dropbox Business**.
7. Paste the team-admin token.
8. Click **Test connection**. Intelligence SOC issues a single 1-row
...
| Intelligence SOC severity | Dropbox `event_type.tag` prefixes |
|---|---|
