...
API** — no Security Hub aggregation in between. This gives Intelligence SOC the
...
region. Repeat per region; each region needs its own Intelligence SOC connector
instance.
...
4. **Test connection** — Intelligence SOC calls `ListDetectors` and confirms the
...
- Archived findings are skipped — if you re-archive in the GuardDuty
  console, Intelligence SOC stops surfacing them on the next poll.

## Severity mapping

GuardDuty publishes severity as a **float in the range 1.0–8.9**. Intelligence SOC's
canonical ladder is `info | low | medium | high`. We bucket using AWS's
own published thresholds:

| GuardDuty score | Intelligence SOC severity |
|---|---|
...
aggregates member-account findings into a single detector. Intelligence SOC supports
both topologies:
...
**No findings appear in Intelligence SOC even though GuardDuty shows them** — by
default Intelligence SOC filters out **archived** findings. Verify the findings
