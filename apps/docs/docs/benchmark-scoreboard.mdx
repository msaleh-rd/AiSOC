...
description: Auto-updated weekly scoreboard of Intelligence SOC eval runs — date, agent version, MITRE accuracy, MTC p50/p95, total USD cost, total tokens. Substrate rows are deterministic CI gates (clearly labelled); wet-eval rows are appended by the T5.5 weekly job. Reproducible, append-only, public.
---
...
Append-only weekly history of every published Intelligence SOC eval run. Each row is one
...
not real bills. Wet-eval rows (real agent, real LLM) are appended by
[`scripts/wet_eval_append_scoreboard.py`](https://github.com/beenuar/AiSOC/blob/main/scripts/wet_eval_append_scoreboard.py)
every time the [T5.5 weekly job](./benchmark-methodology.md#3-substrate-vs-wet-eval)
runs with a funded LLM key — see the empty-secrets caveat below. The two row
types share the same columns so the table reads uniformly, but **never quote
a substrate row as live agent performance**.
:::

:::info Per-PR freshness gate (Phase E1)
The newest `substrate` row is no longer hand-maintained: `scripts/check_scoreboard.py`
runs in CI on every PR and executes the *live LangGraph agent's* tactic-prediction
logic over the 200-incident corpus with the LLM tier turned off (a
deterministic, offline **substrate self-consistency** run — "live" describes
the real agent code path, not a real LLM call). It **fails the build** if the
published `mitre_accuracy` drifts more than 0.02 from that fresh run, the
JSON breaks its schema, or a `substrate` row is mislabelled. So the headline
substrate number here is provably what the current agent's deterministic
tier scores — not a stale figure, and not a live-LLM accuracy claim.
:::
...
on every docs build via `pnpm --filter @isoc/docs scoreboard:check`.
...
2. **Wet-eval rows (T5.5 weekly job).** The
   [`wet-eval.yml`](./benchmark-methodology.md#3-substrate-vs-wet-eval)
   GitHub Action runs the live agent against the same 200-incident corpus
   on a Monday cadence, captures real latency / token / USD telemetry,
   and opens an auto-PR appending one row to `scoreboard.json` (via
   `scripts/wet_eval_append_scoreboard.py`). Wet-eval rows show up at the
   top of the table and on the trend chart.
...
with the JSON output of `pnpm eval:public` attached and the Intelligence SOC team will
investigate.
...
- Validator: `pnpm --filter @isoc/docs scoreboard:check` →
