...
def _build_context(state: InvestigatorState) -> str:
    ...
    sections = [
        ...
        f"Attack chain (deterministic): {' → '.join(state.forensic.attack_chain) if state.forensic.attack_chain else 'None'}",
...
def _fallback_report(state: InvestigatorState) -> str:
    pkg = state.forensic.forensic_package
    if pkg and pkg.get("markdown_report"):
        det_report = pkg["markdown_report"]
        return f"""{det_report}
...
---
*Generated by AiSOC — deterministic forensic report (LLM offline)*
"""
