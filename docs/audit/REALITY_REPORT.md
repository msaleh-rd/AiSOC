...
| Runs entirely on your infrastructure; no callbacks to a vendor cloud and no data exfiltration | `services/agents/app/privacy/redactor.py` (`Pseudonymizer`) is wired into the two real outbound LLM call sites — `app/api/explain.py::_llm_summary` (on-demand explainer) and `app/agents/auto_triage_agent.py::run_auto_triage` (always-on, highest-volume path). Internal IPs, emails, paths, secrets, internal hostnames, and usernames are pseudonymized before the prompt leaves the process and rehydrated locally before reaching the analyst. Public/external IOCs are intentionally not redacted (documented trade-off). | PARTIAL (no longer OVERCLAIM) | `test_llm_egress_redaction.py` captures the outbound HTTP body of both call sites and asserts planted PII never appears verbatim; `test_privacy_redactor.py` covers the redactor in isolation | Yes — both added to the agents job in `ci.yml` |
...
| Public weekly benchmark scoreboard — same harness, weekly against `main` | `.github/workflows/wet-eval.yml` + `scripts/wet_eval_append_scoreboard.py`; `apps/docs/docs/benchmark-scoreboard.mdx` | functional-untested (previously OVERCLAIM; append path now implemented) | `scripts/check_scoreboard.py --check` validates schema/honesty of whatever `wet_eval_append_scoreboard.py` writes; the append script refuses to publish a `substrate:false` row from anything but a real `mode:"live"` wet-eval run | Yes for the append mechanics; still contingent on `WET_EVAL_OPENAI_KEY`/`AISOC_BENCH_BOT_TOKEN` being funded repo secrets (unverifiable from this workspace) for the weekly cron to actually produce a live row |
...
1. **"No data exfiltration / runs entirely on your infrastructure."** ~~RESOLVED~~ (previously ranked #1). `services/agents/app/privacy/redactor.py` now exists and is wired into both real outbound LLM call sites (`app/api/explain.py`, `app/agents/auto_triage_agent.py`), gated by `test_llm_egress_redaction.py` in CI. Remaining honest gap: this audit covers the two documented, highest-impact call sites (on-demand explain + the always-on auto-triage path); other LLM call sites in the agents codebase (e.g. swarm hypothesis generation, RCA prompts, full investigation graph nodes) have not been individually audited for the same gap and may still send raw evidence — track as a follow-up rather than assuming full coverage.
...
4. ~~RESOLVED~~ **"Public weekly benchmark scoreboard — same harness, weekly against `main`."** (previously ranked #4). `.github/workflows/wet-eval.yml` now appends a real `substrate:false` row to `apps/docs/static/data/scoreboard.json` via `scripts/wet_eval_append_scoreboard.py` (validated by `scripts/check_scoreboard.py --check` in the same job), instead of only rewriting `benchmark.md`. Remaining honest caveat: whether `WET_EVAL_OPENAI_KEY` / `AISOC_BENCH_BOT_TOKEN` are actually funded GitHub secrets cannot be verified from this workspace — if unfunded, the preflight gate (`scripts/wet_eval_check.py`) still no-ops cleanly rather than fabricating a row.
